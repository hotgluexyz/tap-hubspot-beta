...
class FormSubmissionsStream(hubspotV1Stream):
    ...
    def post_process(self, row: dict, context: Optional[dict]) -> dict:
        """As needed, append or transform raw data to match expected structure."""
        # read submittedAt while it is still the raw epoch-millisecond value,
        # before super() converts it into a datetime
...
class MarketingEmailsStream(hubspotV3Stream):
...
class ThreadsStream(hubspotV3Stream):
    ...
    def get_url_params(self, context: Optional[dict], next_page_token: Optional[Any]) -> Dict[str, Any]:
        params = super().get_url_params(context, next_page_token)
        rep_key = self.get_starting_time(context, is_inclusive=True).replace(tzinfo=pytz.utc)
