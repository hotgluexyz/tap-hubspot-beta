...
class hubspotBulkStream(hubspotStream):
    bulk_parent_stream = True
    child_context_key = "ids"
    ...
    def _sync_records(  # noqa C901  # too complex
        ...
        for current_context in context_list or [{}]:
            ...
            child_context_bulk = {self.child_context_key: []}
            ...
            for record_result in self.get_records(current_context):
                ...
                # --- join multiple child contexts for bulk streams ---
                # Sync children, except when primary mapper filters out the record
                if self.stream_maps[0].get_filter_result(record):
                    # add id to child_context_bulk ids
                    child_context_bulk[self.child_context_key].append(child_context[self.child_context_key])
                
                if len(child_context_bulk[self.child_context_key]) >= self.bulk_child_size:
            ...
            if child_context_bulk[self.child_context_key]:
                self._sync_children(child_context_bulk)
            #----
